...
**Still open** (needs a design decision, not more backend code): native OS notifications + a permission-approval window in the tray (today's tray still shows pairing requests as a plain submenu item, not a system notification), a "Connected Applications" management list with history, and the first-run welcome window. See `MILESTONE_3.md`'s pairing section for the original build and this document's history for the full UX proposal.
