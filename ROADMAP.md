...
- [x] **2.2 — Official Runtime Installer**: download → install → start runtime → `npm install @portixone/sdk` → `new Portix()` → `connect()` → `print()`, no manual configuration
  ...
  - [x] Installer (`installer/portixone.iss`, Inno Setup) — compiled and verified end-to-end on this machine: silent install → service auto-installed and running → tray auto-launched → Start Menu + Startup shortcuts created; reinstall over an existing install preserved the running service without duplicating it; full uninstall removed the service, both shortcuts, and every installed file (a first pass left empty folders behind because the tray was still holding file handles open — fixed by killing it before file removal in `[UninstallRun]`)
  ...
  - [ ] Real icon/visual identity (today's tray icon is a solid-color placeholder) and a code-signing certificate (installer is unsigned — triggers SmartScreen)
