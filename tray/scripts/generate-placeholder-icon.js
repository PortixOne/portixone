...
// Functional placeholders so systray2 has icons to load; swap `assets/*.ico`
// for real branding whenever that work happens.
const SIZE = 16;

function buildIco(r, g, b) {
  ...
  return Buffer.concat([iconDir, entry, bmpHeader, xor, and]);
}
...
const icons = {
  'icon.ico': [0x1a, 0x2b, 0x6b], // dark blue — normal/ready state
  'icon-pending.ico': [0xd9, 0x8a, 0x0a], // amber — a pairing request is waiting for approval
};

for (const [filename, [r, g, b]] of Object.entries(icons)) {
  const ico = buildIco(r, g, b);
  writeFileSync(join(outDir, filename), ico);
  console.log(`Wrote ${join(outDir, filename)} (${ico.length} bytes)`);
}
