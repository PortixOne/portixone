...
### Docs a stranger actually follows — Developer-Zero audit

✅ **Blocker found and fixed (2026-07-11), branch `fase-10-onboarding`.** Walked the official docs as an external developer would, using a real published `npm install @portixone/sdk` in a clean project outside the monorepo: the mock quickstart works copy-paste (exit 0). But the root `README.md` headline — and the `LANDING.md` hero — led with `new Portix()`, which only authenticates when the runtime runs with the dev-convention key (`dev-local-key`, `sdk-js/src/portix.ts`). A real installer-based install resolves its admin key to `randomUUID()` (the installer stages only `.env.example`, no `.env`), so the SDK's default `dev-local-key` fails auth and `connect()` throws for a stranger following the docs. It was masked on the dev machine because its runtime already uses `dev-local-key` — a textbook "works on my machine" doc trap. Fixed by switching both to `new Portix({ appId, tenant })` (the auto-pair path that matches `sdk-js/README.md` and works against any install), plus a mock-preview box-alignment fix (`sdk-js/src/mock-preview.ts`). **Still open**: the live `portix.dev` landing needs the same edit (separate repo), and an SDK republish is needed to ship the mock-preview fix to npm users.
